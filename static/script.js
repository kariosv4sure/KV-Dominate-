...
async function fetchCrypto() {
  try {
    ...
    section.innerHTML = data.map(coin => `
      <div class="bg-black/40 p-4 rounded-xl shadow-lg hover:scale-105 transition">
        ...
        <p class="text-${coin.price_change_percentage_24h >= 0 ? 'green' : 'red'}-400 font-semibold">
...
async function fetchNews() {
  try {
    ...
    if (!data.results) return section.innerHTML = "<p>No news available right now.</p>";
    section.innerHTML = data.results.slice(0, 5).map(n => `
      <div class="bg-black/30 p-4 rounded-lg hover:bg-black/50 transition">
        <a href="${n.url}" target="_blank" class="text-orange-400 font-semibold hover:underline">${n.title}</a>
      </div>
    `).join("");
  } catch (e) {
...
async function searchTerm() {
  ...
  try {
    const termRes = await fetch(`/term/${query}`);
  ...
  // 2️⃣ Try coin
  try {
    const coinRes = await fetch(`/crypto/${query}`);
    const coinData = await coinRes.json();

    if (coinData.error) {
      alert("No term or coin found!");
      return;
    }
